...
<div class="@if(isset($entry) && ($entry->priority === \App\Enums\Priority::High || $entry->priority === \App\Enums\Priority::Client)) border-2 border-red-600 @endif rounded-xl px-2.5 py-1.5 space-x-1 flex flex-row items-center transition-opacity @if(isset($entry) && $entry->complete) opacity-40 @endif"
    ...
    @isset($entry)
        ...
        @elseif($entry->priority === \App\Enums\Priority::High)
            <x-icon name="s-star" />
        @elseif($entry->priority === \App\Enums\Priority::Client)
            <x-icon name="s-currency-dollar" />
